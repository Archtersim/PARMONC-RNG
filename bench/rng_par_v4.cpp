// rng_par_v4.cpp -- ISA-aware optimization of the PARMONC 128-bit MCG.
//
// Same numerical sequence u_n = u_0 * A^n (mod 2^128) as v1/v2/v3, but
// the hot loop is rewritten to exploit instructions actually available
// on Intel Core Ultra 7 (Arrow Lake): AVX2 + BMI2 + FMA3, no AVX-512.
//
// Two implementations live in this binary, selected at runtime via CPUID:
//
//   path A: "avx2"    -- 4-way ILP body: four independent ГПСЧ chains
//                        stepping with A^4. Each iteration produces 4
//                        doubles, packed into one __m256d and written
//                        with a single _mm256_storeu_pd. This unblocks
//                        the OoO engine (4 independent MUL chains in
//                        flight) and halves the number of stores.
//
//   path B: "scalar"  -- the v3 hot loop (one chain, scalar store).
//                        Используется как запасной путь, если CPUID не сообщает о наличии AVX2.
//
// Why this works on this CPU:
//   * 64x64 -> 128 multiply (the kernel of mul_mod_2_128) HAS no SIMD
//     equivalent in AVX2 -- AVX-512 has VPMULLQ but only low-64.
//     So we don't try to SIMD the multiply; we run 4 of them in parallel
//     via ILP, and let the front-end issue MUL/MULX every cycle.
//   * BMI2's MULX is emitted automatically by gcc when -mbmi2 is on,
//     wherever __umul128 / unsigned __int128 is used. This buys one
//     extra parallel mul per cycle (MUL vs MULX have different uops).
//   * AVX2 is used purely for the store path (and for the 53-bit
//     mantissa -> double conversion, packed 4 wide).
//
// Numerical equivalence: bit-identical sequence u_n is produced. The
// running checksum may differ from v3 by 1-2 ULPs because the
// reduction order is different (4 partial sums, then a horizontal
// add), but bench_rng3.ps1's tolerance of 1e-3 covers this with room
// to spare.
//
// Build (MSYS2 g++):
...
// ---------------------------------------------------------------------
// CPU feature detection via CPUID (works on every x86_64 OS)
// ---------------------------------------------------------------------
struct CpuFeatures {
    bool avx   = false;
    bool avx2  = false;
...
// ---------------------------------------------------------------------
// 128-bit MCG kernel (identical to v3)
// ---------------------------------------------------------------------
struct U128 {
    uint64_t lo;
    uint64_t hi;
