...
//
// Здесь используется тот же алгоритм горячего цикла, что и в rng_par.cpp:
// каждый поток t проходит свой фрагмент решётки
//   u_n = u_0 * A^n (mod 2^128),
// начиная с состояния
//   u_start[t] = u_0 * A^(t * mu),   где mu = N / T.
//
// Главное отличие от rng_par.cpp, которое и должен показать этот файл, —
// реализация умножения 128x128 -> младшие 128 бит. Здесь используются
// нативные intrinsics (_umul128 в MSVC, unsigned __int128 в GCC/Clang),
// а не ручной вариант 32x32 из rng128.h. Фаза инициализации
// (построение u_start[T]) организована так же, как в rng_par.cpp,
// чтобы сравнение v2 и v3 было чистым тестом
// "ручная реализация против нативной".
//
// Стоимость инициализации: log2(mu) + (T-1) умножений, все на главном
// потоке до входа в параллельную область. Предыдущий вариант делал
// T независимых вызовов pow_mod_2_128(A, t * mu) прямо внутри
// параллельной области. Новый вариант добавляет немного
// последовательной работы, но взамен использует общую таблицу
// u_start, что соответствует стандартной схеме PARMONC.
//
// Все три бинарных файла бенчмарка (baseline, rng_par.cpp и этот файл)
// порождают одну и ту же 128-битную последовательность и, следовательно,
// одну и ту же контрольную сумму. Это проверяется sanity-check'ом
// в bench_rng3.ps1 на старте.
//
