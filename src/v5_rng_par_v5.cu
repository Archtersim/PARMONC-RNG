// rng_par_v5.cu -- CUDA-реализация 128-битного MCG PARMONC.
//
// Порождается та же числовая последовательность
//   u_n = u_0 * A^n (mod 2^128),
// что и в v1..v4, но каждое случайное число вычисляется на GPU.
//
// РАЗМЕЩЕНИЕ В ПАМЯТИ
//
// Потоки внутри одного warp должны записывать данные в соседние позиции
// памяти на каждой итерации. Иначе каждый warp будет делать 32 отдельные
// транзакции в HBM вместо одной coalesced-транзакции на 256 байт, и
// эффективная пропускная способность резко падает. Первая версия файла
// использовала схему "на поток" (поток t записывает позиции
// [t*mu, t*mu+mu)), что приемлемо для маленьких размеров, но упирается
// в предел пропускной способности при N >= 1e8.
//
// Новая схема — warp-local. Warp из 32 lane получает непрерывный блок
// `iters_per_warp * 32` позиций в выходном буфере. На итерации i lane `l`
// записывает значение в позицию `warp_offset + i*32 + l`. Соседние lane
// записывают соседние double, то есть получается одна 256-байтная
// coalesced-запись на архитектурах Ampere/Ada/Hopper.
//
// Каждая lane хранит собственное состояние ГПСЧ. На каждой итерации
// состояние продвигается на A^32 (это значение заранее вычисляется на host),
// поэтому схема "последовательных умножений на A для каждого элемента"
// превращается в "одно умножение на A^32 на шаг warp". Число обновлений
// состояния на один элемент остаётся тем же — меняется только раскладка.
//
// Начальный прыжок для потока:
//   u = u_init * A^(start_pos + 1),
// где start_pos = warp_offset + lane_id. Это ставит первое записываемое
// значение ровно в нужную позицию общей решётки и сохраняет ту же
// последовательность u_n = u_0 * A^n, что и у остальных версий.
//
// Сравнение со скалярным CPU-эталоном (rng_par_v3.cpp) на девяти
// граничных случаях, включая размеры, не кратные 32, показало
// побитовую эквивалентность.
//
// Сборка (CUDA Toolkit 12+):
...
// Умножение на host: переносимый вариант без compiler-specific intrinsics.
// На x86_64 и gcc, и MSVC в итоге сводят его к обычной инструкции MUL.
