...
if (-not (Test-Path $buildPs)) { throw "Missing: $buildPs" }
if (-not (Test-Path $benchPs)) { throw "Missing: $benchPs" }

powershell -ExecutionPolicy Bypass -File $buildPs

$slowdowns = $SlowdownList.Split(",") | ForEach-Object { [int]($_.Trim()) } | Where-Object { $_ -gt 0 }
if ($slowdowns.Count -eq 0) { throw "SlowdownList is empty." }
...
Write-Host "Saved summary: $summaryCsv"
