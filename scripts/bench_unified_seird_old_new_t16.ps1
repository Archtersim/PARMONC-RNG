...
if (-not $OutDir) {
    $OutDir = Join-Path $PSScriptRoot "bench_compare_old_new_t16"
}

function Assert-Exists([string]$PathToCheck) {
    if (-not (Test-Path $PathToCheck)) {
        throw "Файл не найден: $PathToCheck"
    }
}
...
function Run-OldCore {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Старый прогон завершился ошибкой: core=$Core exit=$LASTEXITCODE"
    }
...
$seirdDir = Join-Path $BaseDir "seird"
$oldExe = Join-Path $seirdDir "old_profiled_rngcore_compare.exe"
$newExeByCore = @{
    "v1" = (Join-Path $seirdDir "seird_v1_STRICT.exe")
    "v2" = (Join-Path $seirdDir "seird_v2_STRICT.exe")
    "v3" = (Join-Path $seirdDir "seird_v3_STRICT.exe")
    "v4" = (Join-Path $seirdDir "seird_v4_STRICT.exe")
}
...
foreach ($core in $cores) {
    ...
    if (-not (Test-Path $csv)) {
        throw "Ожидаемый CSV-файл не был создан: $csv"
    }
}
