...
if (-not $Exe) {
    $Exe = Join-Path (Join-Path (Split-Path $PSScriptRoot -Parent) "seird") "seird_v4_STRICT.exe"
}
if (-not $CsvPath) {
    $CsvPath = Join-Path $PSScriptRoot "bench_openmp_results.csv"
}

$threads = $ThreadsList.Split(",") | ForEach-Object { [int]($_.Trim()) } | Where-Object { $_ -gt 0 }
if ($threads.Count -eq 0) {
    throw "Список ThreadsList пуст. Пример: -ThreadsList '1,2,4,8,16'"
}
...
function Run-One {
    ...
    & $ExePath --Nr $NrArg --Tmod $TmodArg --threads $Threads --rng-threads $RngThreadsArg --block $BlockArg --rng-prefetch $RngPrefetchArg --rng-slowdown $RngSlowdownArg | Out-Null
    $sw.Stop()

    if ($LASTEXITCODE -ne 0) {
        throw "Запуск завершился ошибкой для threads=$Threads, код выхода=$LASTEXITCODE"
    }
...
if (-not $baseline) {
    throw "Не найден базовый результат для threads=1."
}
