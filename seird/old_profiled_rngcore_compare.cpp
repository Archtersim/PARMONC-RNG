...
static int g_rng_core_old = RNG_CORE_V1;

static const char* rng_core_label_old() {
    switch (g_rng_core_old) {
        case RNG_CORE_V1: return "v1_digit_step";
        case RNG_CORE_V2: return "v2_manual_mul";
        case RNG_CORE_V3: return "v3_native_mul";
        case RNG_CORE_V4: return "v4_buffered4";
...
static inline U128S mul64x64_manual_old(uint64_t x, uint64_t y) {
...
static inline U128S mul_mod_manual_old(const U128S& a, const U128S& b) {
    U128S ll = mul64x64_manual_old(a.lo, b.lo);
    uint64_t cross = a.lo * b.hi + a.hi * b.lo;
    U128S r;
    r.lo = ll.lo;
    r.hi = ll.hi + cross;
    return r;
...
static inline U128S mul_mod_native_old(const U128S& a, const U128S& b) {
#if defined(_MSC_VER)
    uint64_t p1_hi = 0;
    ...
    U128S r;
    r.lo = (uint64_t)p00;
    r.hi = (uint64_t)((p00 >> 64) + cross);
    return r;
#endif
}

static inline U128S add_shifted_small_old(U128S cur, uint64_t v, int shift) {
    if (shift < 64) {
        uint64_t add_lo = v << shift;
        cur.lo += add_lo;
...
static inline U128S build_A_old() {
    ...
    U128S a = {0, 0};
    for (int i = 0; i < 10; ++i) a = add_shifted_small_old(a, (uint64_t)m[i], shifts[i]);
    return a;
}

static inline double u01_from_hi_old(uint64_t hi) {
    uint64_t m53 = hi >> 11;
    double x = (double)m53 * (1.0 / 9007199254740992.0);
    if (x <= 0.0) x = DBL_MIN;
    if (x >= 1.0) x = nextafter(1.0, 0.0);
    return x;
}

double rnd128_v1_digit_step()
{
...
double rnd128_v2_manual_mul() {
    static U128S state = {1ULL, 0ULL};
    static const U128S A = build_A_old();
    state = mul_mod_manual_old(state, A);
    return u01_from_hi_old(state.hi);
}

double rnd128_v3_native_mul() {
    static U128S state = {1ULL, 0ULL};
    static const U128S A = build_A_old();
    state = mul_mod_native_old(state, A);
    return u01_from_hi_old(state.hi);
}

double rnd128_v4_buffered4() {
    static U128S state = {1ULL, 0ULL};
    static const U128S A = build_A_old();
    static double cache[4] = {0.0, 0.0, 0.0, 0.0};
    static int pos = 0;
    static int size = 0;

    if (pos < size) return cache[pos++];

    U128S s1 = mul_mod_native_old(state, A);
    U128S s2 = mul_mod_native_old(s1, A);
    U128S s3 = mul_mod_native_old(s2, A);
    U128S s4 = mul_mod_native_old(s3, A);
    state = s4;

    cache[0] = u01_from_hi_old(s1.hi);
    cache[1] = u01_from_hi_old(s2.hi);
    cache[2] = u01_from_hi_old(s3.hi);
    cache[3] = u01_from_hi_old(s4.hi);
    pos = 0;
    size = 4;
    return cache[pos++];
}
...
{
    switch (g_rng_core_old) {
        case RNG_CORE_V1: return rnd128_v1_digit_step();
        case RNG_CORE_V2: return rnd128_v2_manual_mul();
        case RNG_CORE_V3: return rnd128_v3_native_mul();
        case RNG_CORE_V4: return rnd128_v4_buffered4();
        default: return rnd128_v1_digit_step();
    }
}
...
int main(int argc, char** argv)
{
    ...
    int threads_arg = 1;
    unsigned long long block_arg = 1000000ULL;
    int rng_slowdown_arg = 1;
    ...
    for (int ai = 1; ai < argc; ++ai) {
        ...
        if (key == "--rng-core") {
            need_value("--rng-core");
            string v = argv[++ai];
            if (v == "1" || v == "v1") g_rng_core_old = RNG_CORE_V1;
            else if (v == "2" || v == "v2") g_rng_core_old = RNG_CORE_V2;
            else if (v == "3" || v == "v3") g_rng_core_old = RNG_CORE_V3;
            else if (v == "4" || v == "v4") g_rng_core_old = RNG_CORE_V4;
            else {
                cerr << "Неизвестное значение --rng-core: " << v << " (используйте v1|v2|v3|v4)" << endl;
                exit(2);
            }
        } else if (key == "--Nr") {
            need_value("--Nr");
            ...
            need_value("--threads");
            threads_arg = atoi(argv[++ai]);
        } else if (key == "--block") {
            need_value("--block");
            block_arg = strtoull(argv[++ai], NULL, 10);
        } else if (key == "--rng-slowdown") {
            need_value("--rng-slowdown");
            rng_slowdown_arg = atoi(argv[++ai]);
        } else if (key == "--help" || key == "-h") {
            cout << "Использование: old_profiled_rngcore_compare.exe [--rng-core v1|v2|v3|v4] [--Nr N] [--Tmod D] [--coef C]\n";
            cout << "Compatibility args accepted and ignored for this последовательный model: --threads --block --rng-slowdown\n";
            return 0;
        }
    }

    if (Nr <= 0 || Tmod <= 0 || coef <= 0.0) {
        cerr << "Invalid arguments: require Nr > 0, Tmod > 0, coef > 0" << endl;
        return 2;
    }

    (void)threads_arg;
    (void)block_arg;
    (void)rng_slowdown_arg;

    cout << "[cfg] rng_core=" << rng_core_label_old()
         << " Nr=" << Nr
         << " Tmod=" << Tmod
         << " coef=" << coef
         << endl;
